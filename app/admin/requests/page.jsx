...
import { 
  UserCheck, Search, Clock, CheckCircle, XCircle,
  Brain, Shield, Lightbulb, Palette, Code, Building2, Eye, Loader2, CreditCard
} from 'lucide-react'
...
export default function AdminRequestsPage() {
  ...
  const { data: pendingUsers, mutate: mutatePending } = useSWR('/api/users?status=pending', fetcher)
      ...
      if (action === 'approved') {
        ...
        if (approvedUser) {
          await fetch('/api/inbox', {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              clubId: approvedUser.club_id,
              subject: 'Welcome to the Club!',
              message: `Congratulations ${approvedUser.name}! Your membership has been approved. You can now login and access all club features.`,
              fromName: 'Club Admin'
            })
          })
        }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Approve or reject student join requests with ID verification</p>
        </div>
      ...
      {filteredUsers.length === 0 ? (
        ...
        <div className="grid gap-4">
          {filteredUsers.map((reqUser) => {
            const ClubIcon = clubIcons[reqUser.club_name] || Building2
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-sm text-primary font-medium">
                            Wants to join {reqUser.club_name}
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              ID Card Verification - {selectedUser?.name}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="font-medium">{selectedUser?.club_name}</div>
            </div>
